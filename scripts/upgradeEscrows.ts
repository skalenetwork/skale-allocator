...
async function main() {
    ...
    if (!process.env.SAFE || !process.env.PROXY_ADMIN || !process.env.PROXIES) {
        ...
        console.log(
            ...
            "npx hardhat run scripts/upgradeEscrows.ts --network custom");
        process.exit(1);
    }
    ...
    const safe = ethers.utils.getAddress(process.env.SAFE);
    const proxyAdminAddress = ethers.utils.getAddress(process.env.PROXY_ADMIN);
    let privateKey = process.env.PRIVATE_KEY;
    if (!privateKey.startsWith("0x")) {
        privateKey = "0x" + privateKey;
    }
    const sender = (new Wallet(privateKey)).connect(ethers.provider);
    ...
    const proxyAdmin = await ethers.getContractAt(proxyAdminFile.abi, proxyAdminAddress);
    ...
    if (!process.env.NEW_IMPLEMENTATION) {
        ...
        const escrowFactory = (await ethers.getContractFactory("Escrow")).connect(sender);
